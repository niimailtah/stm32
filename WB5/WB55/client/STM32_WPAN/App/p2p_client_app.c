...
/* USER CODE BEGIN P2PC_APP_Init_1 */
  ...
  UTIL_SEQ_RegTask( 1 << CFG_TASK_RECEIVE_UART_ID, UTIL_SEQ_RFU, ReceiveHandler);
...
/* USER CODE BEGIN P2P_NOTIFICATION_INFO_RECEIVED_EVT */
    	...
    	HAL_UART_Transmit(&huart1, (uint8_t *)&inputMessage, 1, 10);
	...
	while ((index < BLE_CFG_CLT_MAX_NBR_CB) && (aP2PClientContext[index].state != APP_BLE_IDLE))
	{
		switch(aP2PClientContext[index].state)
		{
			case APP_BLE_DISCOVER_SERVICES:
				HAL_UART_Transmit(&huart1, (uint8_t *)"P2P_Discover_Services\n", 22, 10);
			break;
			case APP_BLE_DISCOVER_CHARACS:
				HAL_UART_Transmit(&huart1, (uint8_t *)"* GATT Discover p2p characteristics\n", 36, 10);
			...
			case APP_BLE_DISCOVER_NOTIFICATION_CHAR_DESC:
				HAL_UART_Transmit(&huart1, (uint8_t *)"* GATT Discover Descriptor of RX notification\n", 46, 10);
			...
			case APP_BLE_ENABLE_NOTIFICATION_DESC:
				HAL_UART_Transmit(&huart1, (uint8_t *)"* GATT Enable Server Notification\n", 34, 10);
				aci_gatt_write_char_desc(aP2PClientContext[index].connHandle,
			                           aP2PClientContext[index].P2PNotificationDescHandle, 2,
			                           (uint8_t *)&enable);
			  aP2PClientContext[index].state = APP_BLE_CONNECTED_CLIENT;
			break;
			case APP_BLE_DISABLE_NOTIFICATION_DESC:
				HAL_UART_Transmit(&huart1, (uint8_t *)"* GATT Disable Server Notification\n", 35, 10);
        aci_gatt_write_char_desc(aP2PClientContext[index].connHandle,
			                           aP2PClientContext[index].P2PNotificationDescHandle, 2,
			                           (uint8_t *)&enable);
        aP2PClientContext[index].state = APP_BLE_CONNECTED_CLIENT;
...
void ReceiveHandler(void)
{
	uint8_t index = 0;
	local[1] = rxUart;
    HAL_UART_Transmit(&huart1, (uint8_t *)&rxUart, 1, 10);
	aci_gatt_write_without_resp(aP2PClientContext[index].connHandle,
	                            aP2PClientContext[index].P2PWriteToServerCharHdle, 2, (uint8_t *) (&local));
	rxUart = 0;
	HAL_UART_Receive_IT(&huart1, (uint8_t *)(&rxUart), 1);
}
