 ...
 * Function pointer
 */
typedef uint8_t (*pFunction)(int argc, char *argv[]);
...
command_entry_s			CLI_commands[MAX_COMMAND_NB];
...
void			cli_add_command			(const char *command, const char *help, pFunction exec);
void 			greet					(void);
pGreetFunction  greet_function;
...
static void cli_history_add(char* buff)
{
    ...
    if (len >= MAX_LINE_LEN) /* command too long */
    {
    	return;
    }
 ...
 *
 */
command_line_s shell_init_imp(command_line_s in)
{
	command_line_s out = in;
...
static void cli_rx_handle(shell_queue_s *rx_buff)
{
    static command_line_buffer_s Handle = {.len = 0, .buff = {0}};
    ...
    while (newChar)
    {
        if (Handle.len < MAX_LINE_LEN) /* check the buffer */
        {
            ...
            /* buffer full */
            break;
        } /* end if(Handle.len < MAX_LINE_LEN) */
    } /* end While(newChar) */
...
//    	greet();
    		 ...
    		 && (Handle.len == 1))
    {
        ...
        Handle.len = 0;
    		 ...
    		 && Handle.len > 1) /* check for the length of command */
    {
		NL1();
		Handle.buff[Handle.len - 1] = '\0';
