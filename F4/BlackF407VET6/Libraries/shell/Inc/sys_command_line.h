 ...
 * Typedefs
 */
typedef void (*pGreetFunction)(void);
typedef struct command_line_s {
	UART_HandleTypeDef *handle_uart;
	uint32_t cli_log_stat;
	pGreetFunction  greet_function;
	char *error;
	char *cli_logs_names[];
} command_line_s;

#define shell_init(...) shell_init_imp((command_line_s){.greet_function=greet, \
                                                        __VA_ARGS__})
/*
 * Variables
 */
...
extern pGreetFunction  greet_function;
