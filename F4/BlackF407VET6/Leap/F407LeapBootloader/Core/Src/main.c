...
void init(void);
void loop(void);
uint8_t run_user_app(int argc, char *argv[]);
void bootloader_jump_to_user_app(void);
/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
const char cli_run_user_app[] = "Run user app."
...
void init()
{
	...
	CLI_INIT(&huart1);
	CLI_ADD_CMD("run", cli_run_user_app, run_user_app);
...
/* USER CODE BEGIN 4 */
inline void bootloader_jump_to_user_app(void)
{
	uint32_t app_address = FLASH_SECTOR6_BASE_ADDRESS;
