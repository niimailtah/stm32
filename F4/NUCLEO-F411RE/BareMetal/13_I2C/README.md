...
Получаем данные акселерометра ADXL345 по I2C и передаем по UART на PC.
...
```
c:\ST\STM32CubeProgrammer\bin\STM32_Programmer_CLI.exe --connect port=swd --download build/i2c.elf -hardRst
```
...
## Примечания
ADXL345 умеет связываться по SPI (3-ware и 4-ware) и по I2C. В данном проекте используется I2C.

## Ссылки
[Working with STM32 and Acceleration Sensor: ADXL345 in SPI mode Part 1](https://blog.embeddedexpert.io/?p=1298)
