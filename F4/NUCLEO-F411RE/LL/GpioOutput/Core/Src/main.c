...
int main(void)
{
  ...
  while (1)
  {
	  // на выводе PC13 высокий уровень, кнопка отжата
	  if( (LL_GPIO_ReadInputPort(GPIOC) & (1 << 13))  != 0 )
	  {
		  LL_GPIO_SetOutputPin(GPIOA, LL_GPIO_PIN_5); // установка
	  }
	  // на выводе PC13 низкий уровень, кнопка нажата
	  else
	  {
		  LL_GPIO_ResetOutputPin(GPIOA, LL_GPIO_PIN_5); // сброс
	  }
	  /* USER CODE END WHILE */
