...
Делаем задержку в 1 микросекунду
...
monitor reset init
monitor flash write_image erase 8_SysTick.elf
monitor reset init
monitor resume
```
...
```
c:\tools\ST\STM32CubeProgrammer\bin\STM32_Programmer_CLI.exe --connect port=swd --download 8_SysTick.elf -hardRst
```
