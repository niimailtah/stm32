...
В UART передаем значение от RTC.
...
monitor reset init
monitor flash write_image erase 15_RTC_Calendar.elf
monitor reset init
monitor resume
```
...
```
c:\tools\ST\STM32CubeProgrammer\bin\STM32_Programmer_CLI.exe --connect port=swd --download 15_RTC_Calendar.elf -hardRst
```
